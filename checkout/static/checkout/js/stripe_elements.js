...
form.addEventListener('submit', function (ev) {
    ...
    // Get value of save info box
    var saveInfo = Boolean($('#id-save-info').attr('checked'));
    // initialize the csrftoken
    var csrfToken = $('input[name="csrfmiddlewaretoken"]').val();
    // Makes a small object to pass this info to the new view
    var postData = {
    ...
    // posts data to the view with above url and object
    $.post(url, postData).done(function () {
        stripe.confirmCardPayment(clientSecret, {
            payment_method: {
                card: card,
                billing_details: {
                    // 'trim' method below eliminates excess whitespace
        ...
        // When page is reloaded the error will be in django messages
        location.reload();
    })
});